...
- The held list gains a geopolitics cluster, filtered to words the safe list lets through so every term can reach the check: `russia`, `russian`, `russians`, `soviet`, `soviet union`, `israel`, `israeli`, `palestine`, `north korea`, `communist`, `hitler`. Left off on purpose because they carry ordinary asks: `war`, `invasion`, `nuclear`, `china`, `chinese`, `korea`, `president`, `election`, `republican`. Words the allowlist already rejects (`putin`, `ukraine`, `gaza`, `hamas`, `nato`, `trump`, `biden`) were not added; `parseMessage` turns them away first. Set on dev (237 terms, 2.2 KB); prod is a to do item.
- A visitor's held ask now says why and what would work. The list is server only, so the composer cannot warn before the post, and the Yours card had shown the bare "Held back by Jev" label since the first commit. New `HeldAskNote` in `Yours.tsx` renders under it for visitors: "The wall does not host this topic for visitors. Sign in and the same ask goes through: Jev judges it, a model answers it, and the wall blurs it for everyone but you." Sign in carries `?next=/`. The guard is `status: blocked` with `judged: false`, which only a held term row has; a hold Jev placed itself has `judged: true` and gets no line, since signing in would not change that verdict. Styled by the shared `.nudge`.
- Verified on dev: `messages:send` with a visitor session and "will russia collapse" stored `blocked`, `judged: false`, no tokens. In the browser, "will soviet union collapse" showed the label and the line with `href="/sign-in?next=%2F"`; "is water wet" went live on the wall with no line and drained on the usual clock. Typecheck and 40 tests clean (09:50).
